...
export class BassVoice extends Instrument {
    ...
    generate() {
        ...
        // Bass waveform approximation
        const waveform = el.add(
            el.cycle(freqNode),
            el.mul(0.5, el.cycle(el.mul(freqNode, 0.5))),
            el.mul(0.25, el.cycle(el.mul(freqNode, 1))),
            el.mul(0.125, el.cycle(el.mul(freqNode, 1.5)))
        );
